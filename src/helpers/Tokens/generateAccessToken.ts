...
export const generateAccessToken = (userId: number, name: string, role: UserRole, token_version: number, experience_id: number | undefined, image?: string): string => {
    ...
    const payload = createTokenPayload({ userId, name, role, experience_id, image }, token_version);
