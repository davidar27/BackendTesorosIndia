...
export const getContentNotification = async (role: 'entrepreneur' | 'client', type: 'Cancelación' | 'Confirmación' | 'Reembolso' | 'General', user_id: number): Promise<string> => {
    ...
    const getContent = () => {
        if (role === 'entrepreneur' && type === 'General') {
            return {
                title: 'Nueva Reserva de Paquete',
            ...
            return {
                ...
                message: `Tienes una notificación pendiente que requiere tu atención.`,
                showConfirm: true,
                showCancel: true
            };
        }
    ...
    if (type === "General") {
        ...
        <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eaeaea; border-radius: 8px;">
            ...
            <div style="text-align: center; margin: 30px 0;">
                ${content.showConfirm ? `
                <a href="${confirmRoute}?token=${userToken}" 
            ...
            <p style="color: #666666; font-size: 14px; line-height: 1.5; margin-bottom: 20px;">
                ${role === 'entrepreneur'
                ? 'Recuerda confirmar o rechazar las reservas en un tiempo prudente para brindar un mejor servicio a tus clientes.'
