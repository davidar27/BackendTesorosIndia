"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createUserRepository = void 0;
const db_1 = __importDefault(require("../../config/db"));
const createUserRepository = (newUser) => __awaiter(void 0, void 0, void 0, function* () {
    const { first_name, last_name, email, password, phone_number } = newUser;
    const sql = 'INSERT INTO usuario (nombre, apellido, correo, contraseña, telefono) VALUES (?, ?, ?, ?, ?)';
    const [result] = yield db_1.default.execute(sql, [first_name, last_name, email, password, phone_number]);
    return result.insertId;
});
exports.createUserRepository = createUserRepository;
